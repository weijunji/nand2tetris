...
pub enum AST {
    ...
    BranchOp(BranchOperator, String),
    FunctionOp(FunctionOperator, String, u16),
}
...
pub fn eat_func_num<T: std::io::Read>(token: &mut lexer::TokenStream<T>) -> Option<(String, u16)> {
    match token.next() {
        Some(Token { token: TokenType::Ident(id), ..}) => {
            let num = match token.next() {
                Some(Token{ token: TokenType::Number(n), .. }) => n,
                Some(t) => {
                    error(token, &t);
                ...
                match t.token {
                    ...
                    TokenType::Ident(id) => {
                        match id.as_str() {
                            ...
                            "return" => single_op!(vec, token, SingleOperator::Ret),
                            "label" => {
                                match token.next() {
                                    Some(Token{ token: TokenType::Ident(id), .. }) => {
                                        vec.push(AST::BranchOp(BranchOperator::Label, id));
                                        eat_line(token, true);
                                    },
                                    Some(t) => {
                                        error(token, &t);
                            ...
                            "if-goto" => {
                                match token.next() {
                                    Some(Token{ token: TokenType::Ident(id), .. }) => {
                                        vec.push(AST::BranchOp(BranchOperator::If, id));
                                        eat_line(token, true);
                                    },
                                    Some(t) => {
                                        error(token, &t);
                            ...
                            "goto" => {
                                match token.next() {
                                    Some(Token{ token: TokenType::Ident(id), .. }) => {
                                        vec.push(AST::BranchOp(BranchOperator::Goto, id));
                                        eat_line(token, true);
                                    },
                                    Some(t) => {
                                        error(token, &t);
                            ...
                            "function" => {
                                match eat_func_num(token) {
                                    Some((func, arg)) => {
                                        vec.push(AST::FunctionOp(FunctionOperator::Func, func, arg));
                                        eat_line(token, true);
                                    }
                                    None => ()
                                }
                            ...
                            "call" => {
                                match eat_func_num(token) {
                                    Some((func, arg)) => {
                                        vec.push(AST::FunctionOp(FunctionOperator::Call, func, arg));
                                        eat_line(token, true);
                                    }
                                    None => ()
                                }
